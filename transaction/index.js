...
class Transaction {
	...
	static validateStandardTransaction({transaction, state}){
		return new Promise((resolve,reject) =>{
			...
			if (toAccount.codeHash) {
				console.log('++++++++++++++++++++++++++++++++++++++++++++++++');
				let storageTrie = state.storageTrieMap[toAccount.codeHash];
	...
	static runTransaction({state, transaction}){
		switch(transaction.data.type){
			...
			console.log(
				' -- Updated account data to reflect the standasrd transaction');
			break;
	...
	static runCreateAccountTransaction({state, transaction}) {
		...
		// console.log('already put create account', state.getAccount({address}))
		console.log('already put create account', state.getAccount({address: codeHash? codeHash : address}))

				
		let storageTrie = state.storageTrieMap[codeHash? codeHash : address];
		console.log('already put create account in storage', storageTrie);
